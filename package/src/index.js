...
const doList = (list, key) => {
  let listData = {}
  list.map(v => {
    const firstChar = v[key].substr(0, 1)
    const indexKey = pinyin(firstChar)[0]
...
const defaultLineFunc = (v, key) => (
  <div className="default-index-list-line">{v[key]}</div>
)
...
const IndexList = ({
  ...
  const { indexList, listData } = doList(list, indexKey)
